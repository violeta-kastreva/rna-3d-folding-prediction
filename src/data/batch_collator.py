...
class BatchCollator:
    NON_PADDED_KEYS: list[DataPointKey] = ["has_msa", "has_product_sequences", "num_ground_truths", "is_synthetic"]
    ...
    def __call__(self, batch: list[DataPoint]) -> DataBatch:
        ...
        collated_data["sequence"], collated_data["sequence_mask"] = self.sequence_padder.pad(
            [item["sequence"] for item in batch], do_calculate_mask=True
        )
        ...
        for key in ("msa", "msa_profiles"):
            collated_data[key] = self.sequence_padder.pad([
                item[key]
                if item[key] is not None else
                torch.empty((0, *batch[0][key].shape[1:]), dtype=batch[0][key].dtype)
                for item in batch
            ],
        ...
        collated_data["ground_truth"] = self.sequence_padder.pad(collated_data["ground_truth"], pad_value=0.0)

        collated_data["product_sequences"] = [item["product_sequences"] for item in batch]
